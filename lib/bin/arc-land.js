...
if (branchType == BRANCH_TYPES.ITERATION) {
  Promise.all([envStatus.getOriginBranchVersion('master'), envStatus.fetchEnvData('staging')]).then(values => {
    ...
    const stagingVersion = values[1]; // staging环境是否异常

    if (stagingVersion !== masterVersion || stagingVersion.err) {
    ...
    envStatus.fetchEnvData('production').then(pVersion => {
      ...
      if (pVersion === masterVersion && envStatus.compareVersion(localVersion, masterVersion) === 1) {
        createLand('master');
      } else {
