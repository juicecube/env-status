...
function getBranchType(branch) {
    if ((/^\d+\.\d+\.\d+$/).test(branch)) {
        if (isValidVersion(branch)) {
            return interfaces_1.BRANCH_TYPES.ITERATION;
        }
        else {
            return interfaces_1.BRANCH_TYPES.OTHERS;
        }
    }
    if ((/^\d+\.\d+\.\d+-feat-.+$/).test(branch)) {
        if (isValidVersion(branch.split('-')[0])) {
            return interfaces_1.BRANCH_TYPES.ITERATION_FEATURE;
        }
        else {
            return interfaces_1.BRANCH_TYPES.OTHERS;
        }
    ...
    if ((/^\d+\.\d+\.\d+-fix-.+$/).test(branch)) {
        ...
        if (isValidVersion(version)) {
            return interfaces_1.BRANCH_TYPES.ITERATION_FIX;
        }
        else if (isValidVersion(version, true)) {
            return interfaces_1.BRANCH_TYPES.HOTFIX;
        }
        else {
            return interfaces_1.BRANCH_TYPES.OTHERS;
        }
...
function getVersionFromBranchName(branch) {
    if (getBranchType(branch) !== interfaces_1.BRANCH_TYPES.OTHERS) {
...
function fetchEnvData(env) {
    return new Promise(function (resolve) {
        ...
        if (!config) {
            resolve({ err: interfaces_1.FETCH_ERR.CONFIG_UNDEFINED, env: env });
            return;
        }
        if (typeof config.url !== 'function') {
            resolve({ err: interfaces_1.FETCH_ERR.URL_FUNCTION_UNDEFINED, env: env });
            return;
        }
        ...
        fetch.fetchUrl("" + url + (url.indexOf('?') > 0 ? '&' : '?') + "t=" + Date.now(), function (error, meta, body) {
            ...
            if (error || meta.status !== 200) {
                data = { err: interfaces_1.FETCH_ERR.LOAD_ERROR, env: env };
            }
            else {
                try {
                    ...
                    data = { err: interfaces_1.FETCH_ERR.PARSE_RESPONSE_ERROR, env: env };
                }
            }
