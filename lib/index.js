...
var _envDataCache = {};
var _fetchOriginPromise;
function _fetchOrigin() {
    if (_fetchOriginPromise) {
        return _fetchOriginPromise;
    }
    return _fetchOriginPromise = new Promise(function (resolve, reject) {
...
function getLastCommit() {
    var jsonStr;
    try {
        jsonStr = child_process_1.execFileSync('git', ['show', '--stat', '--format={"commit": "%h", "author": "%an", "branch": "%d"}|||']).toString().split('|||')[0];
    }
    catch (err) {
...
function compareVersion(a, b) {
    ...
    for (var i = 0, l = aArr.length; i < l; i++) {
        var ai = parseInt(aArr[i]);
        var bi = parseInt(bArr[i]);
        if (ai > bi) {
            return 1;
        }
        else if (ai < bi) {
...
function isEnvAvailable(env) {
    return Promise.all(['production', 'staging', env].map(function (env) { return fetchEnvData(env); })).then(function (envsData) {
        var envData = envsData[2] || {}, stgData = envsData[1] || {}, prdData = envsData[0] || {};
        if (!envData.version) {
            return false;
        }
        if (envData.env == 'production') {
            ...
            if (compareRes == 0) {
                ...
                if (compareRes == 0) {
                    ...
                    return _fetchOrigin().then(function () {
                        try {
